...
export type PublishHistoryItem = { id: number; project_id: string; wiki_title: string; version_name: string; action: string; release_status: string; file_status: string; wiki_status: string; index_status: string; mail_status: string; error_message: string; logs: string[]; created_at: string; updated_at: string }
...
export async function getMailHistory(params: { project_id?: string; wiki_title?: string; limit?: number }): Promise<{ ok: boolean; items: MailHistoryItem[] }> { const { data } = await http.get('/api/mail/history', { params }); return data }

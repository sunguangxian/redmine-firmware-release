...
export async function getPublishHistory(params: { project_id?: string; wiki_title?: string; version_name?: string; limit?: number }): Promise<{ ok: boolean; items: PublishHistoryItem[] }> { const { data } = await http.get('/api/releases/publish-history', { params }); return data }

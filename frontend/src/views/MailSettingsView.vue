    ...
    <el-card v-if="settings?.is_admin" class="card">
      ...
      <div class="form-grid">
        ...
        <div class="full-row contact-template-list">
          <div v-for="(item, index) in internalPeople" :key="index" class="contact-row">
            <el-input v-model="item.name" placeholder="姓名" />
            <el-input v-model="item.email" placeholder="邮箱" />
            <el-button @click="internalPeople.splice(index, 1)">删除</el-button>
          </div>
          <el-button @click="addInternalPerson">新增内网联系人</el-button>
        </div>
      </div>
...
function addInternalPerson() { internalPeople.value.push({ name: '', email: '' }) }

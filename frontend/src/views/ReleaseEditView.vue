  ...
  <div>
    ...
    <el-card class="card">
      ...
      <div class="toolbar" style="margin-top: 16px">
        ...
        <el-button v-if="selectedWikiTitle && noticeEnabled" :disabled="busy" :loading="retryingNotice" type="warning" @click="retryNotice">{{ canRetryNotice ? '重发邮件' : '单独发送通知邮件' }}</el-button>
...
function onDocumentClick(event: MouseEvent) { const target = event.target instanceof HTMLElement ? event.target : null; const tab = target?.closest('.el-tabs__item'); if (!tab || tab.classList.contains('is-active')) return; if (!isDirty.value && !publishing.value) return; if (!window.confirm('当前页面有未保存的编辑内容，确认切换页面吗？')) { event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation() } else { baseline.value = snapshot(); emit('dirty-change', false) } }
function statusLabel(row: unknown, field: 'release_status' | 'file_status' | 'wiki_status' | 'index_status' | 'mail_status'): string { const record = row as Record<string, unknown>; return String(record[`${field}_label`] || record[field] || '') }
...
function buildNoticeFormData(sendType = 'manual'): FormData { const data = new FormData(); data.append('project_id', projectId.value); data.append('wiki_title', selectedWikiTitle.value); data.append('version_name', form.version_name); data.append('mail_scope', mailScope.value); data.append('mail_to', [mailTo.value.join(','), manualMailTo.value].filter(Boolean).join(',')); data.append('mail_cc', [mailCc.value.join(','), manualMailCc.value].filter(Boolean).join(',')); data.append('mail_subject', mailSubject.value); data.append('mail_body', mailBody.value); data.append('send_type', sendType); selectedFiles.value.forEach((file) => data.append('files', file)); return data }
